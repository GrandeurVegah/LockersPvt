...
public class HandleOptions {
    public static void handleWelcomeScreenInput() {
        ...
        // text scanner which can parse primitive types and strings
        Scanner sc = new Scanner(System.in);
        // evaluates the expression at the bottom of the loop instead of the top
        // loop executed at least once
        do {
            try {
                MenuOptions.displayMenu(); // display optins the user has
                int input = sc.nextInt();

                switch (input) {
                    case 1: // if user wants to display files
                        FileOperations.displayAllFiles("main");
                        break;
                    case 2: // if input is valid
                        HandleOptions.handleFileMenuOptions();
                        break;
                    case 3: // if user wants to exit program
                        ...
                        break;
                    default:
                        System.out.println("Please select a valid option from above."); // loop starts here then works
                                                                                        // and workd its way up
                }
            } catch (Exception e) { // print out error the call the handleWelcomeScreenInput() funtion again
                System.out.println(e.getClass().getName());
                handleWelcomeScreenInput();
            }
        } while (running == true); // allows the funtion to run on an infinte loop
    }
    ...
    public static void handleFileMenuOptions() {
        ...
        do {
            ...
            } catch (Exception e) { // print out error the call the handleFileMenuOptions() funtion again
                System.out.println(e.getClass().getName());
